...
/* ─────────────────────────────────────────────
   HTTP SERVER — file statis + REST API /api/orders
   (frontend sekarang pakai HTTP polling, sama seperti di Vercel,
   supaya perilaku lokal & production identik)
───────────────────────────────────────────── */
let SEQ = 0;
...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath === '/api/orders') {
    ...
    if (req.method === 'GET') {
      sendJson(res, 200, { seq: SEQ, orders: ORDERS, storage: 'file' });
      return;
    }
    if (req.method === 'POST') {
      try {
        const { action, room, oid, payload } = await readBody(req);
        if (!oid) { sendJson(res, 400, { error: 'oid wajib diisi' }); return; }
        ...
        SEQ++;
        saveOrders();
        // Tetap broadcast lewat WebSocket juga (untuk klien lama jika ada)
        broadcast({ type: 'event', action, room, oid, payload });
        sendJson(res, 200, { ok: true, seq: SEQ });
      } catch (e) {
